...
중간 실패 시에도 완성된 연도는 즉시 패널에 체크포인트된다. ZIP 원본 조각만
저장되고 패널 병합 전에 중단된 경우에는 다음 `resume` 실행이 저장된 원본 조각을
직접 읽으므로 해당 ZIP을 다시 다운로드하지 않는다. 서로 다른 헤더를 가진 TXT가
한 ZIP에 함께 있어도 원본 조각의 `tables` 배열에 각각 보존한다.

맨 앞의 OpenDART 기업 고유번호 ZIP은 일시적인 네트워크 오류에 대비해 최대 5회
재시도한다. `resume`에서 재시도가 모두 실패하면 기존 패널에 저장된 종목코드와
고유번호 매핑을 사용해 벌크 수집을 계속한다. `rebuild`는 기존 패널을 버리는
작업이므로 이 대체 경로를 사용할 수 없다.
