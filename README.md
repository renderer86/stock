...
상단 작업공간 탭에서 수집 결과를 용도별로 확인할 수 있습니다.

- `가치평가`: ROE·ROA 필터, 적정가 시나리오, DART 주요주주
- `국내 수급·공매도`: 투자자별 순매수 차트·종목 순위, 공매도 거래·잔고
- `공시·임원매수`: 자사주·배당·수주·CB 금액 상세, 확인된 임원매수와 후보, 최근 공시
- `미국 시장`: 종목 검색·가격 차트·기술지표, Finnhub 추천·기업지표, FINRA 공매도
...
시장 카드의 작은 차트는 항상 보이고 확대 차트는 카드를 선택할 때만 열립니다. 뉴스는
원문 페이지의 Open Graph 이미지를 찾을 수 있는 경우 기사 썸네일도 함께 표시합니다.
...
FnGuide, OpenDART, 뉴스·AI 브리핑, manifest 작업이 이어지므로 전체 실행의 마지막 단계는
아닙니다.
OpenDART 상세 단계도 배당·수주 원문, 자사주·CB 구조화 API, 임원·주요주주 지분변동의
3단계로 나누어 처리 건수·결과 건수·오류 수·경과 시간을 출력합니다.
